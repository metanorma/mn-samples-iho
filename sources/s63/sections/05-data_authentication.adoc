...
Data authentication is provided using a digital signature compliant with the Digital Signature Standard (DSS) <<dss,[2]>>. The DSS uses the Secure Hash Algorithm (SHA-1) <<sha,[3]>> to create a message digest (hash). The message digest is then input to the Digital Signature Algorithm (DSA) <<dss,[2]>> to generate the digital signature for the message using an asymmetric encryption algorithm and the 'private key' of a key pair. Asymmetric algorithms have the property that data encrypted using the 'private key' of the key pair can only be decrypted using the 'public key' of the key pair.
...
P, Q and G are numerical parameters used in the Digital Signature Algorithm as input to the key creation process. Each data server can use a different set of P, Q and G or use an existing set to generate random key pairs. The Digital Signature Standard <<dss,[2]>> describes their derivation and use.
...
The SA Digital Certificate will be in X509v3 format <<ITU-T_X.509,[4]>> and represents a DSA Public Key of length 512 bits. The SA Digital Certificate will always be available in a file called IHO.CRT. The IHO.CRT file is available from IHO at https://iho.int/[www.iho.int].
